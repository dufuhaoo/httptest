...
from ApiManager.utils.operation import add_project_data, add_module_data, add_case_data, add_config_data, \
    add_register_data,sz_callback_data,pay_callback_data,sq_callback_data,sq_complaint_callback_data,sz_complaint_callback_data,coupon_recharge_data,\
    cc_driver_callback_data,sq_callback_data_for_channel,sz_callback_for_pipe_data
...
def sz_call_back_logic(**kwargs):
    """
    神州回调信息逻辑处理
    :return:
    """
    if kwargs.get('bank') is '':
        return '银行不能为空'
    if kwargs.get('tradeOrderId') is '':
        return '订单号不能为空'
    if kwargs.get('callBackStatus') is '':
        return '回调状态不能为空'
    if kwargs.get('driverId') is '':
        return '车型不能为空'
    if kwargs.get('actualKiloLength') is '':
        return '行驶公里数不能为空'

    return sz_callback_data(**kwargs)

def cc_driver_callback_logic(**kwargs):
    """
    曹操司机端操作逻辑处理
    :param kwargs:
    :return:
    """
    if kwargs.get('event') is '':
        return '事件不能为空'

    return cc_driver_callback_data(**kwargs)

def sq_call_back_logic(**kwargs):
    """
    首汽订单回调信息逻辑处理-直接回调pipe
    :return:
    """
    if kwargs.get('tradeOrderId') is '':
        return '订单号不能为空'
    ...
    return sq_callback_data(**kwargs)

def sq_call_back_logic_for_channel(**kwargs):
    """
    首汽订单回调信息逻辑处理-通过渠道回调
    :return:
    """
    ...
    if kwargs.get('callBackStatus') is '':
        return '回调状态不能为空'
    if kwargs.get('carGroupId') is '':
        return '车型组不能为空'
    return sq_callback_data_for_channel(**kwargs)

def sz_callback_for_pipe_logic(**kwargs):
    """
    神州订单回调pipe逻辑处理
    :param kwargs:
    :return:
    ...
    if kwargs.get('callBackStatus') is '':
        return '回调状态不能为空'
    return sz_callback_for_pipe_data(**kwargs)

def sq_complaint_call_back_logic(**kwargs):
    """
    首汽工单回调信息逻辑处理
    :return:
    """
    if kwargs.get('tradeOrderId') is '':
        return '订单号不能为空'
    if kwargs.get('amount') is '':
        return '金额不能为空'
    if kwargs.get('refundType') is '':
        return '类型不能为空'

    return sq_complaint_callback_data(**kwargs)

def sz_complaint_call_back_logic(**kwargs):
    """
    神州工单回调信息逻辑处理
    :return:
    """
    if kwargs.get('tradeOrderId') is '':
        return '订单号不能为空'
    if kwargs.get('refundAmount') is '':
        return '金额不能为空'
    if kwargs.get('isFinish') is '':
        return '是否完结不能为空'

    return sz_complaint_callback_data(**kwargs)

def coupon_recharge_logic(**kwargs):
    """
    Spdb红包充值逻辑处理
    :param kwargs:
    :return:
    """
    if kwargs.get('eventId') is '':
        return '活动ID不能为空'
    if kwargs.get('userId') is '':
        return '用户ID不能为空'

    return coupon_recharge_data(**kwargs)

def pay_callback_logic(**kwargs):
    """
    订单支付回调信息逻辑处理
    :return:
    """
    ...
    return pay_callback_data(**kwargs)
