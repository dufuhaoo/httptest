...
def get_time(period=None,op=None,value=None):
    """
    获取时间
    :return:
    """
    ...
    if not period and not value:
        return today.strftime(time_format)
    if period == 'm':
        if op == 'before':
            return (today + datetime.timedelta(minutes=-int(value))).strftime(time_format)
        elif op == 'after':
            return (today + datetime.timedelta(minutes=+int(value))).strftime(time_format)

    elif period == 'h':
        if op == 'before':
            return (today + datetime.timedelta(hours=-int(value))).strftime(time_format)
        elif op == 'after':
            return (today + datetime.timedelta(hours=+int(value))).strftime(time_format)
        ...
        if op == 'before':
            ...
            return (today + datetime.timedelta(days=+int(value))).strftime(time_format)
...
def trade_random_login(user_mobile=None,user_type=None):
    ...
    response = requests.post(url=login_url, data=base64.b64encode(json.dumps(data)), headers={'Content-Type':'application/x-www-form-urlencoded'})
...
def sheet_login(user_type='admin'):
    ...
    response = requests.get(url=url,headers={'content-type': 'application/json; charset=UTF-8'})
...
def clean_activity_rule():
    ...
    :return:
    """
    mysql_execute('delete  from tbl_trade_activity_rules_config;')
