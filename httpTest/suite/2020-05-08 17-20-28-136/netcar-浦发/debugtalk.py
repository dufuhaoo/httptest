...
def trade_random_login(user_mobile=None, user_type=None):
    ...
    if user_mobile:
        data = {'respCode': '0000', 'respMsg': '', 'interfaceId': '', 'version': '', 'resDateTime': '', 'signType': '',
                'sign': '',
                'outUuId': '', 'data.code': user_mobile, 'data.bizParams': ''}
    else:
        random_mobile = create_phone_number()
        data = {'respCode': '0000', 'respMsg': '', 'interfaceId': '', 'version': '', 'resDateTime': '', 'signType': '',
                'sign': '', 'outUuId': '', 'data.code': random_mobile, 'data.bizParams': ''}

    response = requests.post(url=login_url, data={'data':base64.b64encode(str(data).encode("utf-8"))},
