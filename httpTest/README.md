...
6. 命令行窗口切换到HttpRunnerManager目录 生成数据库迁移脚本,并生成表结构
    ```bash
        /usr/local/python3/bin/python3 manage.py makemigrations ApiManager #生成数据迁移脚本
        /usr/local/python3/bin/python3 manage.py migrate  #应用到db生成数据表
    ```
...
8. 启动服务,
    ```bash
        /usr/local/python3/bin/python3 manage.py runserver 192.168.0.94:8888
    ```

9. 启动worker, 如果选择同步执行并确保不会使用到定时任务，那么此步骤可忽略
    ```bash
        /usr/local/python3/bin/python3 manage.py celery -A HttpRunnerManager worker --loglevel=info  #启动worker
        /usr/local/python3/bin/python3 manage.py celery beat --loglevel=info #启动定时任务监听器
